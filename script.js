...
function formatNumber(num, showDecimal, limitDecimals) {
  if (!showDecimal) return Math.round(num).toString().replace('.', ',');
  const formatted = showDecimal ? num.toFixed(limitDecimals ? 2 : undefined) : Math.round(num);
...
function toLatex(matrix) {
  ...
  return `\\begin{bmatrix}${matrix.map(
    row => row.map(val => formatNumber(val, showDecimal, limitDecimals)).join(' & ')
  ).join(' \\\\ ')}\\end{bmatrix}`;
}
...
function multiplyScalar() {
  ...
  showLatex(`${scalar} \\cdot ${toLatex(A)} = ${toLatex(result.map(row => row.map(num => formatNumber(num, document.getElementById('decimal')?.checked, document.getElementById('limitDecimal')?.checked))))}`);
}
...
function powerMatrix() {
  try {
    ...
    showLatex(`${toLatex(matrixA.toArray())}^${n} = ${toLatex(result.toArray().map(row => row.map(num => formatNumber(num, document.getElementById('decimal')?.checked, document.getElementById('limitDecimal')?.checked))))}`);
  } catch (err) {
...
function addCheckboxListener(id) {
  document.getElementById(id)?.addEventListener('change', () => {
    const output = document.getElementById('latexOutput');
    const lastLatex = output?.dataset?.lastLatex;
    if (lastLatex) showLatex(lastLatex);
  });
}
    ...
    try {
      ...
      switch (action) {
        ...
        case 'inv':
          ...
          try {
            result = math.inv(A);
            showLatex(`${toLatex(A)}^{-1} = ${toLatex(result.map(row => row.map(num => formatNumber(num, document.getElementById('decimal')?.checked, document.getElementById('limitDecimal')?.checked))))}`);
          } catch (err) {
        ...
        case 'trans':
          ...
          showLatex(`${toLatex(A)}^T = ${toLatex(result.map(row => row.map(num => formatNumber(num, document.getElementById('decimal')?.checked, document.getElementById('limitDecimal')?.checked))))}`);
          break;
        case 'add':
          try {
            ...
            result = math.add(A, B);
            showLatex(`${toLatex(A)} + ${toLatex(B)} = ${toLatex(result.map(row => row.map(num => formatNumber(num, document.getElementById('decimal')?.checked, document.getElementById('limitDecimal')?.checked))))}`);
          } catch (err) {
            showLatex(`\\text{${err.message.replace(/_/g, '\\_')}}`);
          }
          break;
        ...
        case 'mul':
          try {
            ...
            showLatex(`${toLatex(A)} \\times ${toLatex(B)} = ${toLatex(result.map(row => row.map(num => formatNumber(num, document.getElementById('decimal')?.checked, document.getElementById('limitDecimal')?.checked))))}`);
          } catch (err) {
            showLatex(`\\text{${err.message.replace(/_/g, '\\_')}}`);
          }
          break;
        case 'obe':
          result = applyOBE(A);
          showLatex(`\\text{Hasil OBE dari } ${toLatex(A)} = ${toLatex(result.map(row => row.map(num => formatNumber(num, document.getElementById('decimal')?.checked, document.getElementById('limitDecimal')?.checked))))}`);
          break;
        case 'oke':
          result = applyOKE(A);
          showLatex(`\\text{Hasil OKE dari } ${toLatex(A)} = ${toLatex(result.map(row => row.map(num => formatNumber(num, document.getElementById('decimal')?.checked, document.getElementById('limitDecimal')?.checked))))}`);
          break;
      }
    } catch (err) {
...
addCheckboxListener('limitDecimal');